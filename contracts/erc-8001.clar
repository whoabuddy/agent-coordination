...
(define-private (contains-fold-step
    (elem principal)
    (accum {
      found: bool,
      target: principal,
  ...
  {
    found: (or (get found accum) (is-eq elem (get target accum))),
    target: (get target accum),
  }
)

(define-private (contains-principal?
    (plist (list 20 principal))
    (p principal)
  )
  (get found
    (fold contains-fold-step plist {
      found: false,
      target: p,
    })
  )
...
(define-private (buff21-lt-fold-step
    (idx uint)
    (accum {
      res: (optional bool),
      a: (buff 21),
...
(define-private (buff21-lt?
    (a (buff 21))
    (b (buff 21))
  )
  (match (get res
    (fold buff21-lt-fold-step
      (list
...
(define-private (sorted-fold-step
    (i uint)
    (accum {
      sorted: bool,
      plist: (list 20 principal),
    ...
    (if (not curr-sorted)
      ...
      (if (>= i (- curr-n u1))
        ...
        (let (
          ...
          {
            sorted: (and curr-sorted (buff21-lt? curr-b next-b)),
            plist: curr-plist,
            n: curr-n,
          }
        )
...
(define-constant DOMAIN_NAME_HASH  ;; sha256(DOMAIN_NAME_ASCII_STR)(sha256 DOMAIN_NAME_STR))
(define-constant DOMAIN_VERSION_HASH  ;; sha256(DOMAIN_VERSION_ASCII_STR)(sha256 DOMAIN_VERSION_STR))
...
(define-constant AGENT_INTENT_TYPEHASH  ;; sha256(AGENT_INTENT_TYPE_ASCII_STR)(sha256 AGENT_INTENT_TYPE_STR))
...
(define-constant ACCEPTANCE_TYPEHASH  ;; sha256(ACCEPTANCE_TYPE_ASCII_STR)(sha256 ACCEPTANCE_TYPE_STR))
...
(define-constant VERIFYING_CONTRACT_HASH  ;; sha256(VERIFYING_CONTRACT_ASCII_STR)(sha256 VERIFYING_CONTRACT_STR))
...
(define-private (get-domain-separator)
  (let ((chain32 (buff32FromUint64 chain-id)))
    (sha256 (concat DOMAIN_NAME_HASH
      (concat DOMAIN_VERSION_HASH (concat chain32 VERIFYING_CONTRACT_HASH))
    ))
  )
)

;; Private: concat step for participant hash160s
(define-private (participant-hash-fold-step
    (h (buff 20))
    (accum (buff 400))
  )
  (unwrap-panic (as-max-len? (concat accum h) u400))
)
...
(define-private (participants-to-hash (participants (list 20 principal)))
  (sha256 (fold participant-hash-fold-step (map principal-hash160 participants)
    (unwrap-panic (as-max-len? 0x u400))
  ))
)
...
(define-private (intent-struct-hash
  ...
  (sha256 (concat AGENT_INTENT_TYPEHASH
    (concat payload-hash
      (concat (buff32FromUint64 expiry)
        (concat (buff32FromUint64 nonce)
          (concat (address-to-buff32 agent)
            (concat coord-type (concat (buff32FromUint64 coord-value) part-hash))
          ))
      ))
  ))
)
...
(define-private (acceptance-struct-hash
  ...
  (sha256 (concat ACCEPTANCE_TYPEHASH
    (concat intent-hash
      (concat (address-to-buff32 participant)
        (concat (buff32FromUint64 accept-nonce)
          (concat (buff32FromUint64 accept-expiry) conditions)
        ))
    )))
)
...
(define-private (acceptance-digest
  ...
  (let (
      ...
      (struct-h (acceptance-struct-hash intent-hash participant accept-nonce accept-expiry
        conditions
      ))
    )
...
(define-public (propose-coordination
  ...
  (let (
    ...
    (let ((prev-nonce (default-to u0 (map-get? agent-nonces { agent: agent }))))
      ...
      (let (
          ...
          (intent-hash (intent-struct-hash payload-hash expiry nonce agent coord-type
            coord-value part-hash
          ))
        )
        (asserts! (is-none (map-get? intents { intent-hash: intent-hash }))
          ERR_INVALID_PARTICIPANTS
        )
...
(define-public (accept-coordination
  ...
  (let (
    ...
    (let ((intent (unwrap! intent-opt ERR_NOT_FOUND)))
      (asserts! (> (get expiry intent) now) ERR_EXPIRED)
      (asserts! (is-eq (get status intent) PROPOSED) ERR_INVALID_STATE)
      (asserts! (contains-principal? (get participants intent) caller)
        ERR_NOT_PARTICIPANT
      )
      (asserts!
        (is-none (map-get? acceptances {
          intent-hash: intent-hash,
          participant: caller,
        }))
      ...
      (let (
          (accept-nonce u0)
          (digest (acceptance-digest intent-hash caller accept-nonce accept-expiry
            conditions
          ))
        ...
        (let (
          ...
          (asserts! (is-eq (unwrap! signer-opt ERR_INVALID_SIG) caller)
            ERR_INVALID_SIG
          )
          (map-insert acceptances {
            intent-hash: intent-hash,
            participant: caller,
          } {
            accept-expiry: accept-expiry,
          ...
          (let (
            ...
            (let ((acceptance-h (acceptance-struct-hash intent-hash caller accept-nonce
                accept-expiry conditions
              )))
              (print {
...
(define-private (fresh-fold-step
    (p principal)
    (accum {
      fresh: bool,
      now: uint,
  ...
  (if (not (get fresh accum))
    accum
    (let ((acc-opt (map-get? acceptances {
        intent-hash: (get intent-hash accum),
        participant: p,
      })))
      (match acc-opt
        acc {
          fresh: (and (get fresh accum) (>= (get accept-expiry acc) (get now accum))),
          now: (get now accum),
          intent-hash: (get intent-hash accum),
        }
        {
...
(define-private (all-acceptances-fresh?
  ...
  (get fresh
    (fold fresh-fold-step participants {
      fresh: true,
      now: now,
      intent-hash: intent-hash,
    })
...
(define-public (execute-coordination
  ...
  (let (
    ...
    (let ((intent (unwrap! intent-opt ERR_NOT_FOUND)))
      ...
      (asserts!
        (all-acceptances-fresh? intent-hash (get participants intent) now)
        ERR_ACCEPT_EXPIRED
      )
      (asserts! (is-eq (sha256 payload) (get payload-hash intent))
        ERR_PAYLOAD_HASH_MISMATCH
      )
      (let ((new-intent (merge intent { status: EXECUTED })))
...
(define-public (cancel-coordination
  ...
  (let (
    ...
    (let (
      ...
      (asserts! (or (is-eq tx-sender agent) (> now (get expiry intent)))
        ERR_UNAUTHORIZED
      )
      (map-set intents { intent-hash: intent-hash }
        (merge intent { status: CANCELLED })
      )
      (print {
...
(define-private (is-accepted?
    (intent-hash (buff 32))
    (p principal)
  )
  (is-some (map-get? acceptances {
    intent-hash: intent-hash,
    participant: p,
  }))
)
...
(define-private (accepted-filter-step
    (p principal)
    (accum {
      accepted: (list 20 principal),
      intent-hash: (buff 32),
    })
  )
...
(define-read-only (get-accepted-by
    (intent-hash (buff 32))
    (participants (list 20 principal))
  )
  (get accepted
    (fold accepted-filter-step participants {
      accepted: (unwrap-panic (slice? participants u0 u0)),
      intent-hash: intent-hash,
    })
  )
)
  ...
  (let (
    ...
    (match intent-opt
      intent (let (
          ...
          (effective-status (if (or (is-eq stored-status EXECUTED) (is-eq stored-status CANCELLED))
            stored-status
            (if (> now expiry)
              EXPIRED
              stored-status
